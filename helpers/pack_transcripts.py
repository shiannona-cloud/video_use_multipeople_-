...
def group_into_phrases(
    ...
    Transcript `words` entries have types 'word', 'spacing', or 'audio_event'.
...
def render_markdown(entries: list[tuple[str, float, list[dict]]], silence_threshold: float) -> str:
    ...
    for name, duration, phrases in entries:
        ...
        for p in phrases:
            ...
            if spk is not None:
                # Some ASR pipelines return IDs like "speaker_0"; strip the prefix.
                spk_str = str(spk)
...
def main() -> None:
    ap = argparse.ArgumentParser(description="Pack transcripts into takes_packed.md")
