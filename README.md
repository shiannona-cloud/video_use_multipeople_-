...
```bash
# 1) Put this folder under your project as:
#    .cursor/skills/video-use/
...
yt-dlp --version           # optional (download online sources)

# 4) Configure ASR models (optional override)
cp .env.example .env
```

If you want global (all-projects) availability in Cursor, copy this folder to:
...
# 3. Configure ASR models (optional override)
cp .env.example .env
$EDITOR .env                  # FUNASR_MODEL / FUNASR_VAD_MODEL / FUNASR_PUNC_MODEL
```
...
For dialogue-heavy footage (consulting, counseling, interview), use a **topic-first flow**:

1. transcribe + pack to `takes_packed.md`
2. output topic candidates with timestamps and speaker roles
3. let user pick topics
4. generate EDL only from selected topics while preserving question-answer completeness
...
**Layer 1 — Audio transcript (always loaded).** One funASR pass per source gives word-level timestamps, speaker tags (when available), and audio-event-like markers from the transcript stream. All takes pack into a single ~12KB `takes_packed.md` — the LLM's primary reading view.
...
Use [`DISTRIBUTION.md`](./DISTRIBUTION.md) for a copy-paste distribution guide, including:

- install paths for Cursor and Claude
- dependency checklist
- a PowerShell packaging script that exports a clean zip
