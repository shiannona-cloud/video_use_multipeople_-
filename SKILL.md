...
13. **For dialogue content, output a topic plan before any edit.** The user selects topics first, then editing begins.
...
21. **Rough cuts are recall-first, not precision-first.** Expand each topic range with safety handles and semantic boundary snapping so clips do not lose context at the head/tail.
...
└── edit/
    ...
    ├── rough_cuts/              ← pre-selection coarse cuts split by topic
...
- funASR models configured via env (optional override): `FUNASR_MODEL`, `FUNASR_VAD_MODEL`, `FUNASR_PUNC_MODEL`.
...
3. **Topic-first analysis (required for interviews/counseling/dialogue).** Read `takes_packed.md` and output a topic plan before editing:
   - topic label
   - problem statement from seeker/user side
   - counselor/host response summary
...
4. **Export readable topic sheet (required for counseling/dialogue).** Write `<edit>/topic_outline_only.txt` with one block per topic in this order:
   ...
   - `关键词: ...`
   - `发言数: ...`
   - `话题概述: ...` (1–2 sentences, readable, not keyword soup)
   - `代表发言:` (2–4 representative lines)
5. **Generate rough cuts before topic selection (required for counseling/dialogue).** Using `topic_outline_only.txt`, split the original source into one coarse clip per topic and export to `<edit>/rough_cuts/`.
   - one clip per topic range
   - keep source timeline order (no narrative re-order yet)
   - coarse segmentation only (selection-stage material, not final cut)
   - use `ffmpeg` for automated batch splitting (no manual timeline cutting)
   - use fast encode if needed for quick user review
   - apply safety handles on both sides before export (recommended default: +2.0s head and +2.0s tail; clamp to source bounds)
   - after handle expansion, snap boundaries to nearest safe speech boundary/silence (do not cut in the middle of a word/sentence)
   - if adjacent topics are too close (e.g., gap <= 1.0s), allow slight overlap/bridge to avoid missing transitional context
6. **Rough-cut naming convention (required).**
   - filename format: `TopicNN_<easy_to_understand_topic_name>.mp4`
   - include topic index (`Topic01`, `Topic02`, ...)
   - topic name comes from `话题概述`, rewritten as a short user-facing phrase
   - sanitize to filesystem-safe characters while keeping readability
7. **Converse.** Present this topic sheet and the rough-cut folder in plain English and ask the user to select which topics are worth editing.
8. **Propose strategy.** 4–8 sentences: selected topics, cut direction, continuity handling, subtitle style, optional animation/grade, length estimate. **Wait for confirmation.**
9. **Execute.** Produce `edl.json` based on selected topics. Drill into `timeline_view` at ambiguous moments. Build animations in parallel sub-agents when requested. Apply grade per-segment. Compose via `render.py`.
10. **Preview.** `render.py --preview`.
11. **Self-eval (before showing the user).** Run `timeline_view` on the **rendered output** (not the sources) at every cut boundary (±1.5s window). Check each image for:
   - Visual discontinuity / flash / jump at the cut
   - Waveform spike at the boundary (audio pop that slipped past the 30ms fade)
   - Subtitle hidden behind an overlay (Rule 1 violation)
   - Overlay misaligned or showing wrong frames (Rule 4 violation)
   - Dialogue breakage (question without answer, or answer detached from question)
...
12. **Iterate + persist.** Natural-language feedback, re-plan, re-render. Never re-transcribe. Final render on confirmation. Append to `project.md`.

For rough cuts (selection stage), run a quick boundary QA pass before presenting:
- verify each clip has non-zero duration and expected time span (`ffprobe`)
- verify first/last utterance is not obviously truncated at the boundary
- if truncation risk is detected, auto-expand boundary and regenerate that clip
- write `<edit>/rough_cuts/manifest.md` with: topic id, requested range, padded/snapped range, output filename, duration

## Topic plan output (for dialogue/counseling material)

Before any cut decisions, output a shortlist of topics with timestamps and speaker roles so the user can choose what to keep.
...
[
  {
    "topic": "sleep anxiety after layoffs",
    "problem_summary": "Seeker reports persistent insomnia and fear after recent layoffs.",
    "counselor_summary": "Counselor reframes uncertainty and gives a 3-step sleep stabilization routine.",
    "evidence": [
      {"speaker": "seeker", "start": 132.4, "end": 165.8, "quote": "..."},
      {"speaker": "counselor", "start": 166.0, "end": 248.6, "quote": "..."}
    ],
    "recommended_range": {"start": 130.0, "end": 252.0},
    "completeness_check": "Question and response are both fully covered."
  }
]
```

Selection rule: if the user picks topics A/C/F, only then build EDL ranges from those topics.

Priority override for counseling content: if a topic is counselor commentary on social news, mark it as high-priority and include it by default in the recommended keep set unless the user explicitly opts out.
...
- Before any user topic selection, always export per-topic coarse clips to `<edit>/rough_cuts/`.
- Rough-cut filenames must include topic index + readable short topic name derived from `话题概述`.
- Rough-cut production is automation-only: use `ffmpeg`; never instruct manual rough cutting.
- Rough cuts must preserve context with boundary padding + snapping; prefer extra context over accidental truncation.
...
- **Running Whisper locally on CPU.** Slow and it normalizes fillers. Use the configured funASR pipeline instead.
