...
For counseling/dialogue projects, keep editorial priority consistent with the parent `video-use` skill: counselor commentary on social news is default high-priority keep content, unless the user explicitly asks to remove it.

For counseling/dialogue projects that pass through `video-use` before animation, require the same pre-edit topic sheet output:
- Always generate `<edit>/topic_outline_only.txt` before topic selection.
- Keep per-topic field order fixed: `Topic` -> `时间` -> `关键词` -> `发言数` -> `话题概述` -> `代表发言`.
- Place `话题概述` before `代表发言` for readability.
- Use this sheet as the single source of truth for choosing which dialogue sections receive animation overlays.
